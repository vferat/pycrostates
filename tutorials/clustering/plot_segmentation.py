...
# please refers to the :ref:`Fit <fit>`
# tutorial.
...
# predict the microstates segmentation using the :meth:`pycrostates.cluster.ModKMeans.predict`
...
# which happens during periods of low signal to noise ratio. For each timepoint,
# this algorithm takes into account the label of the neighboring segments in order to favor label continuity
# (i.e. long semgents). The ``factor`` parameter controls the strength of the influence of the neighboring segments
# while the ``half_window_size`` parameter controls how far (in time) neighbors should have influence.
...
#%%
# Parameters usually studied in the analysis of microstates can be computed using the 
...
#%%
# This method returns a `numpy.array` of shape (``n_clusters``, ``n_clusters``), where each element
