...
def _compare_infos(cluster_info, inst_info):
    ...
    if not all(
        kind1 == kind2 for kind1, kind2 in zip(cluster_kinds, inst_kinds, strict=False)
    ):
        logger.warning(
            "Instance to segment into microstates sequence does not have "
    ...
    if not all(
        unit1 == unit2 for unit1, unit2 in zip(cluster_units, inst_units, strict=False)
    ):
        logger.warning(
            "Instance to segment into microstates sequence does not have "
    ...
    if not all(
        f1 == f2 for f1, f2 in zip(cluster_coord_frame, inst_coord_frames, strict=False)
    ):
        logger.warning(
            "Instance to segment into microstates sequence does not have "
