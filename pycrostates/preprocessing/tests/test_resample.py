...
def test_resample_n_resamples_coverage(inst, replace, n_resamples, cov):
    ...
    resamples = resample(inst, n_resamples=n_resamples, coverage=cov, replace=replace)
    n_ch = _picks_to_idx(inst.info, None, exclude="bads").size
...
def test_resample_n_samples_coverage(inst, replace, n_samples, cov):
    ...
    resamples = resample(inst, n_samples=n_samples, coverage=cov, replace=replace)
    n_ch = _picks_to_idx(inst.info, None, exclude="bads").size
    assert isinstance(resamples, list)
    n_data = inst.times.size
    if isinstance(inst, BaseEpochs):
...
def test_n_resamples_n_samples_coverage_errors():
    ...
    with pytest.raises(ValueError, match="'n_resamples' must be a strictly positive"):
    ...
    with pytest.raises(ValueError, match="'n_samples' must be a strictly positive"):
...
def test_resample_random_state():
    """Test resampling with n_samples and coverage provided."""
    resamples_0 = resample(raw, n_resamples=1, n_samples=500, random_state=42)[0]
    resamples_1 = resample(raw, n_resamples=1, n_samples=500, random_state=42)[0]
