...
from pycrostates.cluster.base import BaseCluster


def test_check_n_clusters():
    """Test the checker for n_clusters argument."""
    n_clusters = BaseCluster._check_n_clusters(1)
    assert n_clusters == 1
    with pytest.raises(TypeError, match="'n_clusters' must be an instance of"):
        BaseCluster._check_n_clusters(True)
    with pytest.raises(ValueError,
                       match="number of clusters must be a positive integer"):
        BaseCluster._check_n_clusters(-101)


def test_check_reject_by_annotation():
    """Test the checker for reject_by_annoation argument."""
    reject_by_annotation = BaseCluster._check_reject_by_annotation(True)
    assert reject_by_annotation == 'omit'
    reject_by_annotation = BaseCluster._check_reject_by_annotation(False)
    assert reject_by_annotation is None
    reject_by_annotation = BaseCluster._check_reject_by_annotation(None)
    assert reject_by_annotation is None

    with pytest.raises(TypeError,
                       match="'reject_by_annotation' must be an instance of"):
        BaseCluster._check_reject_by_annotation(1)
    with pytest.raises(ValueError,
                       match="'reject_by_annotation' only allows for"):
        BaseCluster._check_reject_by_annotation('101')
...
def test_reject_short_segments():
    ...
    segmentation = BaseCluster._reject_short_segments(segmentation, data, 3)
