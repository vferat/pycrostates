...
.. glossary::
    ...
    global field power
        ...
        standard deviation for ``eeg`` data, and  as the root mean square for
        ``grad`` and ``mag`` data. GFP is computed at each time point, thus
        it is a one-dimensional time series capturing the spatial variability of the
